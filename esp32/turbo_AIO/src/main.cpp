...
Adafruit_NeoPixel streetLigh(9, STREET_PIN, NEO_GRB + NEO_KHZ800);
void setLight(int color);
void updateBluetooth(void);
void debugFonction(void);
void openGate(void);
void closeGate(void);
...
void debugFonction()
{
  static long millisTimer = 0;
  static int compteurSeconde = 0;
  if(millis() - millisTimer >= 250)
  {
    millisTimer = millis();
    compteurSeconde ++;

    if(compteurSeconde == 4*1)
    {
      openGate();
    }
    else if(compteurSeconde == 4*5)
    {
    }
    else if(compteurSeconde == 4*8)
    {
      closeGate();
    }
    else if(compteurSeconde == 4*15)
    {
      compteurSeconde = 0;
    }
  }
